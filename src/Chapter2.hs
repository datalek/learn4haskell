...
subList :: Int -> Int -> [a] -> [a]
subList x y l =
  let validInput = x >= 0 && y >= 0
      nOfElem = y - x + 1
  in if (validInput) then take nOfElem (drop x l) else []
...
the compiler warns you if you forget to cover some cases (Question: There is a way to bring the warning into an error?). So you
...
-}
rotate :: Int -> [Int] -> [Int]
rotate n xs = take size infinite
  where size      = length xs
        infinite  = if (n >= 0)
                    then drop n (cycle xs)
                    else []
