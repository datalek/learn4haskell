...
Doesn't compile because the type of `x` is (Maybe a) but the fmap
must return a value of (Maybe b).
...
instance Monad List where
  ...
  (>>=) xs f =
    flat (fmap f xs)
    where flat :: List (List a) -> List a
          flat Empty = Empty
          flat (Cons x xs) = x `append` (flat xs)
